...
def UPDATENO(k,num_no):
    com = 'UPDATE "Info" SET no = '+"'"+str(num_no)+"'"+" WHERE spec = '"+str(k)+"'"
    return com

def UPDATEYES(k,num_yes):
    com = 'UPDATE "Info" SET yes = '+"'"+str(num_yes)+"'"+" WHERE spec = '"+str(k)+"'"
    return com

def CreateTable():
    com = 'CREATE TABLE "Info" (spec VARCHAR(1000),no INTEGER, yes INTEGER,budget_mest INTEGER)'
    return com
...
cnxnInfo = pyodbc.connect("Driver={SQL Server Native Client 10.0};"
                            "Server=WIN-K1RC9U9RIIK\SQLS;"
                            "Database=Info;"
                            "Trusted_Connection=yes;")
cursorInfo = cnxnInfo.cursor()
...
class Filter:
    ...
    def main(self):
        ...
        for k in tablesUgatu_sogl:
            ...
            cursorUGATU.execute(select_yes)
            num_yes = 0
            for row in cursorUGATU.fetchall():
                num_yes += 1
            select_no = Select_(k,'No')
            cursorUGATU.execute(select_no)   
            num_no = 0
            for row in cursorUGATU.fetchall():
                num_no += 1
            Insirt = UPDATENO(k,num_no)
            Insiirt = UPDATEYES(k,num_yes)
            cursorInfo.execute(Insirt)
            cursorInfo.execute(Insiirt)


        for k in tablesBGU_sogl:
            select_yes = Select_(k,'Yes')
            cursorBGU.execute(select_yes)
            num_yes = 0
            for row in cursorBGU.fetchall():
                num_yes += 1
            select_no = Select_(k,'No')
            cursorBGU.execute(select_no)   
            num_no = 0
            for row in cursorBGU.fetchall():
                num_no += 1
            Insirt = UPDATENO(k,num_no)
